...
So on the first iteration being the first time it loops it will go to one and print one, then, it will go over to two  and print two.
NOTE: Because strings are similar to lists, I can loop through the characters of the string Too!

                    Break and Continue  (Part 1)
The break statement allows me to instantly terminate a loop before it is completed.
Example:
    count = 1
    while count <= 5:
        if count == 3:
...
The continue statement allows you to skip through one iteration of the loop and restart the loop on the next iteration.
Example:
